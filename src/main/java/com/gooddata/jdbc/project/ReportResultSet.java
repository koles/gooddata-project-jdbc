...
public class ReportResultSet implements ResultSet {
	...
	ReportResultSet(final ProjectStatement statement, final String sql) {
		this.gdc = (this.statement = statement).gdc; // evil one-liner ;)
		this.sql= sql;
		execute();
	}
	...
	private void execute() {
		String resultUri = gdc.executeReport(sql);
		byte[] resultBytes = gdc.exportReportResult(resultUri, "csv");
		InputStream is = new ByteArrayInputStream(resultBytes);
		this.csv = new CSVReader(new InputStreamReader(is));
	}
